...
add_action('wp_enqueue_scripts', function() {
    ...
    wp_enqueue_style('wstudio-approval-style', $base . 'css/approval.css', [], filemtime($path . 'css/approval.css'));
    ...
    wp_enqueue_script('wstudio-approval-ui', $base . 'js/approval-ui.js', ['jquery'], filemtime($path . 'js/approval-ui.js'), true);
    wp_enqueue_script('wstudio-approval-actions', $base . 'js/approval-actions.js', ['jquery', 'wstudio-approval-ui'], filemtime($path . 'js/approval-actions.js'), true);
    ...
    wp_localize_script('wstudio-approval-actions', 'wstudioAjax', [
        'ajax_url'   => admin_url('admin-ajax.php'),
        'nonce'      => wp_create_nonce('wstudio_approve_nonce'),
        'gallery_id' => isset($post->ID) ? intval($post->ID) : 0,
        'labels'     => [
            'approve_button' => get_option('wstudio_approval_button_label', 'Accepter billederne'),
            'processing'     => 'Indlæser...'
        ]
    ]);
});
...
add_shortcode('kundegalleri', function($atts) {
    ...
    $accepted = get_post_meta($post_id, 'wstudio_accepted_time', true);
    ...
    <div class="gallery-info-text">
        <?php echo wp_kses_post(get_option('wstudio_approval_text', 'Når du har godkendt billederne, vil du få mulighed for at downloade dem.')); ?>
    </div>
        ...
        <div id="kundegalleri-container" class="lightgallery kundegalleri-wrapper">
            ...
            if (is_dir($basedir)) {
                ...
                foreach ($files as $file_path) {
                    ...
                    $url = apply_filters('wstudio_get_image_url', $baseurl . $filename, $post_id, $filename);
                    $web_url = apply_filters('wstudio_get_image_url', $weburl . $filename, $post_id, $filename);
                    $orig_url = apply_filters('wstudio_get_image_url', $origurl . $filename, $post_id, $filename);
                    ?>
